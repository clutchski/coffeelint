...
(function() {
  ...
  LexicalLinter = (function() {
    ...
    LexicalLinter.prototype.lintClass = function(token) {
      ...
      if (!regexes.pascalCase.test(className)) {
        attrs = {
          context: "class name: " + className
        };
        return this.createLexError('pascal_case_classes', attrs);
      } else {
        return null;
      }
    };
  ...
  mergeDefaultConfig = function(userConfig) {
    var config, migrate, migration_assistant, rule, ruleConfig;
    config = {};
    migration_assistant = {
      camel_case_classes: function(cfg) {
        userConfig['pascal_case_classes'] = cfg;
        return ['camel_case_classes', 'pascal_case_classes'];
      }
    };
    migrate = function(cfg, ast) {
      var a, b, rest, rule, _ref, _results;
      _results = [];
      for (rule in ast) {
        rest = ast[rule];
        if (!(rule in cfg)) {
        ...
        if (typeof rest === 'function') {
          _ref = rest(cfg[rule]), a = _ref[0], b = _ref[1];
          _results.push(console.log("" + a + " is depricated, see " + b + "."));
        } else {
          _results.push(migrate(cfg[rule], ast[rule]));
        }
      }
      return _results;
    };
